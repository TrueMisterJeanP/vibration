...
assert.match(criticalSplashStyle, /html\[data-theme="light"\]\s*\{[^}]*--avatar-bg:\s*#3d816b[^}]*--startup-surface:\s*#f8f9fa[^}]*background-color:\s*#f8f9fa/);
assert.match(criticalSplashStyle, /html\.ios-pwa-starting,\s*html\.ios-pwa-starting body\s*\{[^}]*background:\s*var\(--startup-surface\)/);
...
assert.match(css, /html\.ios-pwa-starting,\s*html\.ios-pwa-starting body\s*\{[^}]*background:\s*var\(--startup-surface\)/);
assert.match(css, /html\.ios-pwa-starting #startup-splash\s*\{[^}]*position:\s*fixed[^}]*width:\s*var\(--startup-screen-width, 100vw\)[^}]*height:\s*var\(--startup-screen-height, 100vh\)[^}]*background:\s*var\(--startup-surface\)/);
...
assert.match(theme, /page\.contains\("chat-app-page"\)[\s\S]*resolved === "light" \? "#ffffff" : "#071b24"/);
assert.match(theme, /themeColor\.content = restingColor/);
