...
assert.match(loadMessages, /renderMessages\(prepared\.messages, conversation, prepared\.decrypted, \{\s*waitForPreviews,\s*positionMessages: scrollToBottom,\s*\}\);[\s\S]*scrollMessagesToLatest\(conversationID\)/);
assert.match(loadMessages, /renderMessages\(cachedMessages, conversation, null, \{\s*waitForPreviews,\s*positionMessages: scrollToBottom,\s*\}\);[\s\S]*scrollMessagesToLatest\(conversationID\)/);
