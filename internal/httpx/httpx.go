...
func frenchErrorMessage(status int, message string) string {
	if translated, ok := map[string]string{
		...
		"federated files are not supported":               "Les fichiers fédérés ne sont pas encore pris en charge.",
		"federated message updates are not supported":     "Les modifications de messages fédérés ne sont pas encore prises en charge.",
